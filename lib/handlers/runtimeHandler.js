...
const getNodeIdsFromResult = async runtimeResult => {
  ...
  for (const r of result) {
    ...
    nodeIds.push(
      (await dom.requestNode({ objectId: r.value.objectId })).nodeId,
    );
  }
  return nodeIds;
};
...
const runtimeCallFunctionOn = async (
  exp,
  executionContextId,
  opt = {},
) => {
...
const findElements = async (exp, arg) => {
  let nodeIds = [];
  const evalFunc = isString(exp)
    ? runtimeEvaluate
    : runtimeCallFunctionOn;
  for (const contextId of executionContextIds) {
    let nodeIdsFromRes = await getNodeIdsFromResult(
...
const activeElement = async () => {
  ...
  for (const executionContextId of executionContextIds) {
    ...
    const activeElementDetails = result
      ? result.result.value
      : undefined;
